...
# Configuração do Jinja2 para carregar os templates de e-mail
email_template = Environment(loader=FileSystemLoader('app/utils/email_templates/'))

# Cliente SES para enviar e-mails
...
# Função para gerar o contexto do e-mail (com medicamentos)
...
# Função para renderizar o template de e-mail
...
def send_email(recipient, subject, template_name):
    ...
    # Chama o repositório para pegar os medicamentos que precisam ser enviados no horário atual
    medications = MedicationRepository.get_medication_by_time(db, [datetime.now().strftime('%Y-%m-%d %H:%M:%S')])

    # Se não houver medicamentos agendados, retorna uma mensagem de erro
    if not medications:
        db.close()
        return "No medications scheduled for today."

    # Prepara o contexto do e-mail (com nome de usuário e medicamentos)
    context = {
